...
| 5 | /var/log/secure | security related log such as login attempt and sudo usage ; use in RHEL/CentOS/Amazon Linux |
| 6 | /var/log/dmesg | Hardware related message |
...
| 16 | `sudo grep -r 'ssh' /var/log` | searches for occurrences of the string "ssh" in all files within the `/var/log` directory recursively, displaying any matches |
| 17 | `sudo less /var/log/secure` | allows you to view the contents of the `secure` log file, which contains security-related logs like SSH authentication attempts, sudo command usage, and other system security activities. |
| 18 | less /var/log/messages
 | open the log file in a paginated view, allowing you to scroll through the logs to check system events, warnings, or errors. |
...
| 21 | -F option | deal for monitoring log files that are subject to regular rotation or replacement, such as those in `/var/log` |
| 22 | tail -F /var/log/secure | **follow the secure log file** in real-time |
